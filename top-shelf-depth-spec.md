...
| **4 ✅ DONE** | Conveyor mode ("The Belt") | largest | **Built + browser-verified (2026-06-24).** Self-contained real-time mode (own `CV` state, feed loop, render, pointer handling — never touches `G`). Menu "The Belt" → a belt feeds complete triples (always sortable); drag belt→shelf or shelf→shelf; 3-match clears; overflow → cozy "belt backed up" card → restart. Personal best in `save.bestBelt`. Spawn ~2.2s, speeds up with score. SW cache → v28. |
